...
import { IGroup, IJwt, IZabo, IZaboMap } from "types/index.d";
import { IState } from "types/store.d";

import { CHECK_AUTH } from "store/reducers/auth";

const decodedTokenSelector: Selector<IState, IJwt | undefined> = (state: IState) =>
  get(state, ["auth", "jwt"]);
...
export const isAdminOrPendingSelector = (state: IState) => [
  isAdminSelector(state),
  authPendingSelector(state),
];

const zabosSelector: ParametricSelector<IState, string[], [IZaboMap, string[]]> = (
  state: IState,
  zaboIds: string[],
): [IZaboMap, string[]] => [get(state, ["zabo", "zabos"]), zaboIds];
const zabosComputer = ([zabos, zaboIds]: [IZaboMap, string[]]) =>
  zaboIds.map((id: string) => zabos[id]);
...
const isMyZabo = (zabo: IZabo, groups: IGroup[]) =>
  !!groups.find((group: IGroup) => zabo && group._id === get(zabo, ["owner", "_id"]));
