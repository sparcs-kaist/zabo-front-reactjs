...
class ZaboUpload extends PureComponent {
  ...
  _handlePosterAdd = (windowWidth, e) => {
    ...
    const callback = postersURLs => {
      const sorted = postersURLs.sort ((a, b) => (a.index < b.index ? -1 : a.index > b.index ? 1 : 0));
      const extracted = sorted.map (item => item.url);

      this.setState (prevState => {
        if (prevState.posters[0] !== undefined) firstCarouselHeight = prevState.activeCarouselHeight;
    ...
    const getImageHeight = result => {
      ...
      image.onload = () => {
        if (windowWidth < 800)
        // on mobile
        { firstCarouselHeight = ((windowWidth - 50) * image.height) / image.width; } else firstCarouselHeight = (320 * image.height) / image.width;
      };
    };
  ...
  _handlePosterDelete = () => this.setState (prevState => {
    const i = prevState.carouselPos;
    const newPosters = [...prevState.posters]; // [... ] 없으면 prevState 와 같은 객체를 참조 => 업뎃 안함.
    const newPostersPreviewURLs = [...prevState.postersPreviewURLs];
  ...
  _onSubmit = e => {
    ...
    Array.from (this.state.posters).forEach (file => {
      // FileList => Array 로
    ...
    this.props
      .uploadZabo (formData, percentCompleted => this.setState ({ percentCompleted }))
      .then (res => {
  ...
  _onMouseUp = e => {
    // ignore hover when unselecting tag
    ...
    this.state.tags.map (item => (item.tag === clickedText && item.clicked === true ? unactiveTag (e.target) : item));
  }
        ...
        <div className="container">
          <div className="header">
            ...
            <div className="headerLow">
              <div className="headerTitle">Upload Your Poster</div>
          ...
          <div className="inputs">
            <section className="zabo-poster">
              ...
              <div className="posterCarousel">
                {!!postersPreviewURLs[0] === false ? (
                  ...
                  <div>
                    ...
                    <div className="swipeItem" key={url}>
                      <img src={url} width="100%" className="slick-image" />
                      <img
                        src={Delete}
                ...
                {!!postersPreviewURLs[0] === false ? (
                  ...
                  <div className="carousel-navigations">
                    ...
                    key={url + index}
                    className={
                          this.state.carouselPos === index ? 'navigation-selected' : 'navigation'
                        }
                  />
                    ))}
                  </div>
              ...
              />
              <label
                htmlFor={posters[0] === undefined ? 'posterInput' : ''}
                className={
                  posters[0] === undefined
                    ? 'posterContainer inputContainer'
          ...
          <div className="loading-bar">
            <div className="loading-active" style={{ width: `${percentCompleted}%` }} />
