...
export default handleActions(
  {
    [INITIALIZE]: (state, action) =>
      produce(state, (draft: IUploadState) => {
        Object.assign(draft, action.payload);
      }),
    [SET_STEP]: (state, action: Action<any>) =>
      produce(state, (draft: IUploadState) => {
        draft.step = action.payload;
      }),
    [SET_GROUP_SELECTED]: (state, action: Action<any>) =>
      produce(state, (draft: IUploadState) => {
        draft.groupSelected = action.payload;
      }),
    [SET_IMAGES_SELECTED]: (state, action: Action<any>) =>
      produce(state, (draft: IUploadState) => {
        draft.imagesSelected = action.payload;
      }),
    [SUBMIT]: (state, action: Action<any>) =>
      produce(state, (draft: IUploadState) => {
        draft.submitted = action.payload;
      }),
    [SET_IMAGES]: (state, action: Action<any>) =>
      produce(state, (draft: IUploadState) => {
        draft.images = action.payload;
      }),
    [SET_INFO]: (state, action: Action<any>) =>
      produce(state, (draft: IUploadState) => {
        draft.info = action.payload;
      }),
    [SET_MODAL]: (state, action: Action<any>) =>
      produce(state, (draft: IUploadState) => {
